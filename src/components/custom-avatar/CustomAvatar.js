...
const getColorByName = (name) => {
  if (['A', 'N', 'H', 'L', 'Q'].includes(getCharAtName(name).charAt(0))) return 'primary';
  if (['F', 'G', 'T', 'I', 'J'].includes(getCharAtName(name).charAt(0))) return 'info';
  if (['K', 'D', 'Y', 'B', 'O'].includes(getCharAtName(name).charAt(0))) return 'success';
  if (['P', 'E', 'R', 'S', 'U'].includes(getCharAtName(name).charAt(0))) return 'warning';
  if (['V', 'W', 'X', 'M', 'Z'].includes(getCharAtName(name).charAt(0))) return 'error';
  return 'default';
};
